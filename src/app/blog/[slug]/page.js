...
export default function BlogDetailPage({ params }) {
  ...
  return (
    ...
    >
      ...
      <main className="relative bg-[#E7E7E7] min-h-screen flex flex-col items-center px-4 md:px-10 py-32 text-black">
        {/* Floating Arrows */}
        {previousPost && showArrows && (
          <button
            onClick={() => router.push(`/blog/${previousPost.slug}`)}
            className="fixed left-2 top-1/2 transform -translate-y-1/2 z-50 bg-white rounded-full shadow-lg p-2 hover:bg-gray-200 transition-opacity duration-300"
          >
            <ArrowLeft size={24} />
          </button>
        )}
        ...
        {nextPost && showArrows && (
          <button
            ...
            className="fixed right-2 top-1/2 transform -translate-y-1/2 z-50 bg-white rounded-full shadow-lg p-2 hover:bg-gray-200 transition-opacity duration-300"
          >
            <ArrowRight size={24} />
          </button>
        )}
...
<div className="relative p-4">
  <div className="rounded-xl overflow-hidden shadow-lg relative border border-black">
    <div className="relative w-full aspect-[2.4/1]"> {/* 👈 Maintains 1200x500 aspect ratio (2.4:1) */}
      <Image
        src={post.image}
        alt={post.title}
        fill // 👈 Replaces width & height props for full container fill
        className="object-cover"
        sizes="(max-width: 768px) 100vw, 1200px" // Optional: better performance
        priority // Optional: fast load
      />
    </div>
